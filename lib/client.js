...
export class BraveSearchClient {
    ...
    validateParams(service, params) {
        ...
        for (const [key, value] of Object.entries(params)) {
            ...
            if (key === 'context_threshold_mode' && !['strict', 'balanced', 'lenient'].includes(String(value)))
                throw new BraveSearchError('INVALID_PARAMS', 'context_threshold_mode must be strict, balanced, or lenient', { service });
        }
    }
    validateNumber(key, value, service) {
        if (typeof value !== 'number' || !Number.isInteger(value))
            throw new BraveSearchError('INVALID_PARAMS', `${key} must be an integer`, { service });
        const max = key === 'offset' ? 9 : key === 'count' ? (service === 'image' ? 200 : 20) : key.includes('tokens') ? 100_000 : key.includes('urls') ? 50 : 100;
        if (value < 0 || value > max || (key === 'count' && value < 1))
